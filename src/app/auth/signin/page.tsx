	...
	const submitForm = (values: any) => {
		...
		signInWithEmailAndPassword(auth, values.email, values.password)
			...
			.catch((error) => {
				...
				let myErrorMessage = "Login failed";
				switch (error.message) {
					case "Firebase: Error (auth/invalid-credential).":
						myErrorMessage = "Email or Password is incorrect";
						break;
					case "Firebase: Error (auth/operation-not-allowed).":
						myErrorMessage = "Email/password registration is not enabled.";
						break;
					default:
						// Use the original error message or a fallback
						myErrorMessage = error.message || "An unexpected error occurred.";
				}
				toast({
					title: "Login failed",
